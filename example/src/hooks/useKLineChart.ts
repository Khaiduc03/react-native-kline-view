...
import { ThemeManager, DrawTypeConstants, DrawStateConstants } from '../constants';
import {
  generateMockData,
  calculateBOLL,
  calculateMACD,
...
export const useKLineChart = () => {
  ...
  // Select indicator
  const selectIndicator = useCallback((type: 'main' | 'sub', indicator: number) => {
    if (type === 'main') {
  ...
  const processKLineData = useCallback((rawData: KLineData[]): KLineData[] => {
  ...
  const calculateIndicatorsFromTargetList = useCallback((data: KLineData[], targetList: any): KLineData[] => {
    let processedData = [...data];

    // Calculate BOLL
    if (state.selectedMainIndicator === 2) {
      processedData = calculateBOLL(processedData, parseInt(targetList.bollN), parseInt(targetList.bollP));
    }
  ...
  // Pack option list
  const packOptionList = useCallback((processedData: KLineData[]) => {
    const theme = ThemeManager.getCurrentTheme(state.isDarkTheme);
    const targetList = getTargetList();
    ...
    return {
      ...
      drawList: {
        shouldReloadDrawItemIndex: state.selectedDrawTool === DrawTypeConstants.none ? DrawStateConstants.none : DrawStateConstants.showContext,
  ...
  useEffect(() => {
    reloadKLineData();
  }, [state.isDarkTheme, state.selectedMainIndicator, state.selectedSubIndicator, reloadKLineData]);
